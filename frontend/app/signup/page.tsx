    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 400 && errText.includes("already registered")) {
          setError("Account exists. Redirecting...");
  ...
  return (
    <div className="flex min-h-[80vh] items-center justify-center px-4">
      <div className="glass-panel w-full max-w-md p-8 rounded-2xl relative overflow-hidden">
        ...
        <h1 className="text-3xl font-bold mb-2 text-center text-white text-glow">Join Proedit</h1>
        <p className="text-slate-400 text-center mb-6 text-sm">Create your studio account</p>
        ...
        <form onSubmit={submit} className="space-y-5 relative z-10">
          <div>
            ...
            <input className="glass-input w-full" placeholder="you@example.com" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-2">Password</label>
            <input type="password" className="glass-input w-full" placeholder="Create password" value={password} onChange={(e) => setPassword(e.target.value)} />
          </div>
          <button className="btn-primary w-full py-4 text-lg shadow-lg">Create Account</button>
          {error && (
            <div className={`p-3 rounded border text-sm text-center ${error.includes('Redirect') ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
              {error}
            </div>
          )}
        </form>
