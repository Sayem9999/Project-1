...
export default function DashboardPage() {
  ...
  const [activeView, setActiveView] = useState<"projects" | "media">(
    "projects",
  );
    ...
    const load = async () => {
      try {
        const [me, data] = await Promise.all([
          apiRequest<{ id: number; full_name?: string; email?: string }>(
            "/auth/me",
            { auth: true },
          ),
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-6 md:space-y-8 relative group">
      <Spotlight />
      {/* Hero Stats Section */}
        ...
        <div className="lg:col-span-2 glass-panel rounded-3xl p-6 md:p-8 relative overflow-hidden group border-white/5">
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 mb-8 max-w-lg text-sm md:text-base leading-relaxed">
              Your studio is ready. You have{" "}
              <span className="text-white font-bold">{activeCount}</span> active
              projects and{" "}
              <span className="text-white font-bold">{jobs.length}</span> total
              exports.
            </p>
          ...
          {activeView === "projects" && (
            <div className="flex items-center gap-2 pb-4 md:pb-0">
              {["All", "Processing", "Completed"].map((filter) => (
                <button
                  key={filter}
                  className="px-3 py-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-white/5 text-gray-500 hover:text-white transition-all"
                >
                  {filter}
                </button>
              ))}
        ...
        {activeView === "media" ? (
          ...
          <div className="glass-panel border-dashed border-2 border-white/5 rounded-3xl p-12 text-center flex flex-col items-center">
            ...
            <p className="text-gray-400 mb-8 text-sm max-w-xs">
              Upload clips and let our AI agents orchestrate your creative
              vision.
            </p>
            <Link href="/dashboard/upload">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <Link
              ...
              className="aspect-video rounded-3xl border-2 border-dashed border-white/5 hover:border-brand-cyan/50 bg-white/[0.02] hover:bg-white/[0.04] transition-all group flex flex-col items-center justify-center gap-3"
            >
              ...
              <span className="font-black text-gray-500 group-hover:text-white uppercase tracking-[0.2em] text-[10px]">
                New Project
              </span>
            </Link>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute top-4 left-4">
                      <div
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl glass-heavy ${status.bg} border-white/5 shadow-2xl`}
                      >
                        <div
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 transform translate-y-1 group-hover:translate-y-0 transition-transform">
                      <h3 className="text-lg font-black text-white mb-2 leading-tight group-hover:text-brand-cyan transition-colors truncate tracking-tight">
                        Project #{job.id}
                      </h3>
                          ...
                          <div className="flex flex-col gap-1 w-full mt-2">
                            <div className="flex items-center justify-between">
                              <span className="text-brand-cyan animate-pulse">
                                {job.progress_message?.split("]")[1]?.trim() ||
                                  job.progress_message ||
                                  "Processing..."}
                              </span>
                              <span className="text-[9px] text-gray-600 font-black">
                                {job.progress_message?.includes("[AI]")
                                  ? "HOLLYWOOD PIPELINE"
                                  : "AGENTIC WORKFLOW"}
                              </span>
                            </div>
                            <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: job.progress_message?.includes("10%")
                                    ? "10%"
                                    : job.progress_message?.includes("20%")
                                      ? "20%"
                                      : job.progress_message?.includes("50%")
                                        ? "50%"
                                        : job.progress_message?.includes("80%")
                                          ? "80%"
                                          : "15%",
                                }}
                              />
