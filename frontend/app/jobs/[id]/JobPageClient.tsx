...
export default function JobPageClient({ id }: { id: string }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setActionError(
        err instanceof ApiError ? err.message : "Failed to start job",
      );
    }
  };
  ...
  const openEdit = () => {
    ...
    setEditForm({
      ...
      transition_duration: Number(
        job.post_settings?.transition_duration ?? 0.25,
      ),
      ...
      skin_protect_strength: Number(
        job.post_settings?.skin_protect_strength ?? 0.5,
      ),
    });
    setEditError("");
    setShowEdit(true);
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      setEditError(
        err instanceof ApiError ? err.message : "Failed to create edit",
      );
    } finally {
  ...
  const getCurrentStage = (message: string) => {
    ...
    if (
      m.includes("platform") ||
      m.includes("adapt") ||
      m.includes("specialist")
    )
      return 2;
    if (m.includes("render") || m.includes("compiling")) return 3;
    if (m.includes("review") || m.includes("quality") || m.includes("qc"))
      return 4;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl flex items-center gap-3">
            <div
              className={`w-2 h-2 rounded-full ${isProcessing ? "bg-brand-cyan animate-pulse" : isComplete ? "bg-emerald-400" : "bg-gray-600"}`}
            />
            ...
            <div className="flex items-center justify-between relative">
              {stages.map((stage, i) => {
                ...
                return (
                  <div
                    key={stage.key}
                    className="flex flex-col items-center gap-4 relative z-10 flex-1 group"
                  >
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-700 border ${
                        isActive
                          ? "bg-brand-cyan text-black border-brand-cyan shadow-[0_0_30px_rgba(6,182,212,0.4)] scale-110"
                          : isPast
                    ...
                    <span
                      className={`text-[10px] font-black uppercase tracking-widest transition-colors duration-500 ${isActive ? "text-white" : "text-gray-500"}`}
                    >
                      {stage.label}
                    </span>
              ...
              <div className="space-y-3">
                <Link href={`/jobs/${job.id}/download`} className="block">
                  <Button
                    variant="glow"
                    size="lg"
                    className="w-full font-black text-xs uppercase tracking-[0.2em]"
                  >
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {job.scout_result?.assets
                            ...
                            .map((asset: any, i: number) => (
                              <div
                                key={i}
                                className="aspect-video bg-white/5 rounded-lg overflow-hidden border border-white/5 relative group"
                              >
                                {asset.preview_url ? (
                                  <img
      ...
      {showEdit && (
        <div className="fixed inset-0 z-[100] bg-obsidian-950/80 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="w-full max-w-2xl glass-heavy border-white/10 rounded-[40px] p-8 md:p-10 relative overflow-hidden animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="flex items-center justify-between mb-10">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Fine-tune your creative parameters for the next iteration.
                </p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                },
                {
                  ...
                  options: ["platform_default", "broadcast", "social"],
                },
                {
                  key: "color_profile",
                  label: "Color Profile",
                  options: ["natural", "cinematic", "punchy"],
                },
              ].map((field) => (
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((s) => ({
                      ...s,
                      transition_duration: Number(e.target.value) || 0.25,
                    }))
                  }
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((s) => ({
                      ...s,
                      skin_protect_strength: Number(e.target.value) || 0.5,
                    }))
                  }
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-sm text-white focus:outline-none focus:border-brand-cyan/50 transition-colors font-bold tracking-wider"
                />
            ...
            {editError && (
              <div className="mt-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-bold">
                {editError}
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-4 mt-12">
              ...
              <Button
                ...
                className="px-10 font-black text-xs uppercase tracking-widest"
              >
                Execute Edit
              </Button>
            </div>
  ...
  return (
    ...
    >
      <path
        ...
        strokeWidth={2}
        d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944 11.955 11.955 0 012.382 7.984C2.382 7.984 1 18 12 18c11 0 10-10.016 10-10.016z"
      />
    </svg>
  );
}
