...
import { Upload, Sparkles, Zap, Video, CheckCircle2, X, BrainCircuit, ScanSearch, Layers, ArrowLeft, Activity } from 'lucide-react';
    ...
    <div className="min-h-screen bg-obsidian-950 text-white selection:bg-brand-cyan/30 overflow-hidden relative">
      ...
      <div className="relative z-20 flex items-center justify-between p-6 md:p-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Pipeline Operational</span>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-6 pb-32 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-3 mb-6">
            <div className="px-3 py-1 rounded-full bg-brand-cyan/10 border border-brand-cyan/30 text-[10px] font-black uppercase tracking-widest text-brand-cyan">EXPRESS_INITIATOR</div>
            <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">v4.0.2 Stable</span>
          </div>
          <h1 className="text-4xl md:text-7xl font-black tracking-tighter text-white mb-6 uppercase">
            Drop it. <span className="bg-gradient-to-r from-brand-cyan to-brand-violet bg-clip-text text-transparent italic">Vibe it.</span> Done.
          </h1>
          <p className="text-gray-500 max-w-2xl mx-auto text-lg font-bold leading-relaxed">
            The world&apos;s most lethal edit engine. Zero configuration. Maximum output. Upload your footage and witness true multi-agent alignment.
          </p>
        </motion.div>
          ...
          {!file ? (
            ...
            >
              ...
              <div className="w-24 h-24 rounded-[32px] bg-white/5 flex items-center justify-center mb-8 relative z-10 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6">
                <Upload className={`w-10 h-10 ${dragActive ? 'text-brand-cyan animate-bounce' : 'text-gray-600 group-hover:text-brand-cyan'}`} />
              </div>
              <h2 className="text-3xl font-black text-white mb-3 tracking-tight relative z-10">Deploy Raw Media</h2>
              <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] relative z-10">BROWSE_FS OR DRAG_DROP</p>
            ...
            >
              ...
              <div className="text-center">
                <h3 className="text-3xl font-black text-white mb-2 uppercase tracking-tight">Mapping Neural Beats</h3>
                <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Engaging Hollywood Agents to map visual flow...</p>
              </div>
            ...
            >
              ...
              <div className="glass-panel rounded-[40px] p-6 pr-10 flex flex-col md:flex-row items-center gap-8 border-white/5 relative overflow-hidden group">
                ...
                <div className="flex-1 text-center md:text-left py-4">
                  <div className="flex items-center gap-3 mb-3 justify-center md:justify-start">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-400">TELEMETRY_SYNC_OK</span>
                  </div>
                  <h3 className="text-2xl font-black text-white mb-2 tracking-tight truncate max-w-sm">{file.name}</h3>
                  <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest leading-loose">
                    {(file.size / 1024 / 1024).toFixed(1)} MB • 24fps • Intra-frame Mesh detected
                  </p>
              ...
              <div className="space-y-8">
                <div className="flex items-center gap-4 px-2">
                  ...
                  <h3 className="text-xl font-black text-white uppercase tracking-widest">Select Narrative Engine</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {VIBES.map((v) => (
                    ...
                    >
                      ...
                      <div className={`w-12 h-12 rounded-[18px] flex items-center justify-center mb-6 bg-white/5 relative z-10 transition-all duration-700 group-hover/btn:scale-110 group-hover/btn:rotate-12 ${selectedVibe.id === v.id ? 'bg-brand-cyan/10 shadow-lg shadow-brand-cyan/10' : ''}`}>
                        {v.icon}
                      </div>
                      <h4 className="font-black text-white text-lg mb-2 relative z-10 uppercase tracking-tight">{v.label}</h4>
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-relaxed relative z-10">{v.description}</p>
                  ...
                  <span className="relative z-10 flex items-center justify-center gap-4">
                    {uploading ? (
                      <>
                        ...
                        <span>{progress < 90 ? `CALCULATING_ALGORITHMS... ${progress}%` : 'FINALIZING_RENDER...'}</span>
                      </>
                    ) : (
                      <>
        ...
        {!file && (
          <div className="mt-20 flex flex-wrap justify-center gap-16 opacity-30 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-1000">
            <div className="flex items-center gap-3">
              ...
              <span className="font-black tracking-[0.3em] text-[10px] uppercase">Hollywood Agents v4</span>
            </div>
            <div className="flex items-center gap-3">
              <Zap className="w-5 h-5 text-brand-violet" />
              <span className="font-black tracking-[0.3em] text-[10px] uppercase">Neural Color Match</span>
            </div>
            <div className="flex items-center gap-3">
              <Video className="w-5 h-5 text-emerald-400" />
              <span className="font-black tracking-[0.3em] text-[10px] uppercase">Auto QC Integrity</span>
            </div>
          </div>
        )}
      </main>
