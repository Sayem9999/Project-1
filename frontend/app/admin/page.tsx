...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "users" | "jobs" | "credits" | "system"
  >("overview");
  ...
  const [autonomyAuditLogs, setAutonomyAuditLogs] = useState<
    AdminActionLogEntry[]
  >([]);
  ...
  const [jobFilter, setJobFilter] = useState<
    "all" | "processing" | "complete" | "failed"
  >("all");
  ...
  const [architectWorkflowJson, setArchitectWorkflowJson] = useState<
    string | null
  >(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const results = await Promise.allSettled([
        ...
        apiRequest<AutonomyStatus>("/maintenance/autonomy/status", {
          auth: true,
        }),
      ]);
  ...
  const fetchAutonomyStatus = useCallback(async () => {
    ...
    try {
      const [status, audit] = await Promise.all([
        ...
        apiRequest<AdminActionLogEntry[]>(
          "/admin/audit/actions?action_prefix=autonomy&limit=50",
          { auth: true },
        ),
      ]);
      setAutonomy(status);
  ...
  const runAutonomy = useCallback(
    async (opts: { force_heal?: boolean; force_improve?: boolean }) => {
      ...
      try {
        ...
        await apiRequest(`/maintenance/autonomy/run?${query.toString()}`, {
          method: "POST",
          auth: true,
        });
        ...
        toast.error(
          err instanceof ApiError ? err.message : "Autonomy run failed",
        );
      } finally {
  ...
  const setAutonomyProfile = useCallback(
    async (mode: string) => {
      setAutonomyActionLoading(true);
      try {
        const query = new URLSearchParams({ mode });
        const data = await apiRequest<AutonomyStatus>(
          `/maintenance/autonomy/profile?${query.toString()}`,
        ...
        toast.error(
          err instanceof ApiError
            ? err.message
            : "Failed to switch autonomy mode",
        );
      } finally {
        setAutonomyActionLoading(false);
      }
  ...
  const fetchLedger = useCallback(async () => {
    ...
    setLedgerError(null);
    try {
      const data = await apiRequest<CreditLedgerEntry[]>(
        "/admin/credits/ledger?limit=200",
        { auth: true },
      );
      setLedger(data);
    } catch (err: any) {
  ...
  const handleUpdateCredits = async (
    ...
    try {
      await apiRequest(
        `/admin/users/${userId}/credits?credits=${newCredits}&reason=${encodeURIComponent(reason)}`,
        {
          method: "PATCH",
          auth: true,
        },
  ...
  const handleAddCredits = async (
    ...
    try {
      await apiRequest(
        `/admin/users/${userId}/credits/add?delta=${delta}&reason=${encodeURIComponent(reason)}`,
        {
          method: "PATCH",
          auth: true,
        },
  ...
  const handleAdminAction = async (
    jobId: number,
    action: "cancel" | "retry" | "force-retry",
  ) => {
    const toastId = toast.loading(`Executing ${action}...`);
    try {
      await apiRequest(`/admin/jobs/${jobId}/${action}`, {
        method: "POST",
        auth: true,
      });
      fetchJobs(true);
  ...
  const handleCleanup = async () => {
    ...
    try {
      const res: any = await apiRequest("/admin/cleanup", {
        method: "POST",
        auth: true,
      });
      fetchData();
      toast.success("Cleanup complete", {
        id: toastId,
        description: `Freed ${res.mb_freed} MB`,
      });
    } catch (err: any) {
  ...
  const openUserDetails = async (user: UserData) => {
    ...
    try {
      const [jobsData, ledgerData] = await Promise.all([
        apiRequest<JobData[]>(`/admin/users/${user.id}/jobs?limit=10`, {
          auth: true,
        }),
        apiRequest<CreditLedgerEntry[]>(
          `/admin/credits/ledger?user_id=${user.id}&limit=20`,
          { auth: true },
        ),
      ]);
  ...
  const filteredJobs = useMemo(() => {
    ...
    return list.filter((j) =>
      `${j.id} ${j.user_id} ${j.theme} ${j.progress_message}`
        .toLowerCase()
        .includes(q),
    );
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <div className="max-w-md w-full glass-panel border-red-500/20 rounded-[32px] p-10 text-center relative overflow-hidden">
          ...
          <p className="text-gray-500 mb-10 text-sm leading-relaxed relative z-10">
            Administrative oversight requires elevated privileges. Your account
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-gray-500 max-w-2xl text-base md:text-lg leading-relaxed">
              Executive oversight and system governance. Orchestrate resources,
              scale impact, and maintain pipeline health.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <Button
            ...
            className="font-black text-[10px] uppercase tracking-widest border-brand-cyan/20 text-brand-cyan"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6">
        {[
          ...
          },
          {
            icon: Sparkles,
            title: "Pro Volume",
            value: stats?.analytics?.pro_jobs ?? 0,
            sub: `${stats?.analytics?.standard_jobs ?? 0} Standard Jobs`,
            color: "emerald",
          },
          {
            icon: CreditCard,
            title: "Rev. Estimate",
            value: `$${stats?.analytics?.revenue_est ?? 0}`,
            sub: "Projected Gross (Pro)",
            color: "amber",
          },
          {
            icon: Video,
            ...
            sub: `+${stats?.jobs?.recent_24h ?? 0} New Exports`,
            color: "violet",
          },
          {
            icon: HardDrive,
            ...
            sub: `${stats?.storage?.used_gb ?? 0}GB Utilized`,
            color: "amber",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
              <div className="p-5 rounded-[24px] bg-white/[0.03] border border-white/5 hover:border-white/10 transition-colors">
                <div className="text-[10px] font-black text-gray-400 mb-3 flex items-center gap-2 uppercase tracking-widest">
                  <Database className="w-4 h-4 text-brand-cyan" /> Persistence
                  Layer
                </div>
                <div className="flex items-center justify-between">
                  <span
                    className={`text-sm font-black uppercase tracking-widest ${health?.db?.reachable ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {health?.db?.reachable
                      ? "Online_Reachable"
                      : "Persistent_Fail"}
                  ...
                  <div
                    className={`w-2 h-2 rounded-full ${health?.db?.reachable ? "bg-emerald-400" : "bg-red-400"} shadow-[0_0_10px_rgba(52,211,153,0.5)]`}
                  />
                </div>
              </div>
              <div className="p-5 rounded-[24px] bg-white/[0.03] border border-white/5 hover:border-white/10 transition-colors">
                <div className="text-[10px] font-black text-gray-400 mb-3 flex items-center gap-2 uppercase tracking-widest">
                  <Server className="w-4 h-4 text-brand-violet" /> Cache
                  Orchestrator
                </div>
                <div className="flex items-center justify-between">
                  <span
                    className={`text-sm font-black uppercase tracking-widest ${health?.redis?.reachable ? "text-emerald-400" : "text-red-400"}`}
                  >
                  ...
                  <div
                    className={`w-2 h-2 rounded-full ${health?.redis?.reachable ? "bg-emerald-400" : "bg-red-400"}`}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="glass-panel p-8 rounded-[32px] border-white/5 relative overflow-hidden bg-gradient-to-br from-white/[0.02] to-transparent">
              <h3 className="text-[10px] font-black font-bold text-gray-500 uppercase tracking-[0.3em] mb-6">
                Agent LLM Cluster
              </h3>
              <div className="space-y-4">
                {Object.entries(health.llm || {}).map(([name, item]) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-[10px] font-black px-3 py-1 rounded-lg ${item.is_healthy ? "bg-emerald-400/10 text-emerald-400" : "bg-red-400/10 text-red-300"}`}
                    >
      ...
      <div className="flex items-center gap-2 md:gap-4 p-2 glass-panel rounded-[24px] border-white/5 overflow-x-auto no-scrollbar scroll-smooth">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-3 px-6 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap min-w-fit ${
              activeTab === tab.id
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="lg:col-span-2 glass-panel rounded-[32px] overflow-hidden border-white/5">
              ...
              <div className="p-4 space-y-3">
                {jobs.slice(0, 6).map((job) => (
                  <div
                    key={job.id}
                    className="flex items-center justify-between p-5 rounded-3xl bg-white/[0.02] hover:bg-white/[0.04] transition-all group border border-transparent hover:border-white/5"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center font-mono text-[10px] text-brand-cyan group-hover:scale-110 transition-transform">
                        #{job.id}
                      </div>
                      <div>
                        <div className="text-sm font-black text-white mb-1 group-hover:text-brand-cyan transition-colors">
                          {job.theme.toUpperCase()} • {job.id}
                        </div>
                        <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest line-clamp-1">
            ...
            <div className="glass-panel rounded-[32px] overflow-hidden border-white/5">
              ...
              <div className="p-4 space-y-3">
                {users.slice(0, 6).map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between p-5 rounded-3xl bg-white/[0.02] border border-transparent hover:border-white/5"
                  >
                    <div className="flex items-center gap-4">
                    ...
                    <div className="px-3 py-1 rounded-lg bg-amber-400/10 text-amber-400 text-[10px] font-black">
                      {user.credits} CR
                    </div>
                  </div>
                ))}
        ...
        {activeTab === "users" && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="glass-panel rounded-[32px] overflow-hidden border-white/5"
          >
          ...
          >
            <div className="p-8 border-b border-white/5 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
              <div>
                ...
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                  System Load: {processingCount} Parallel Agents
                </p>
              </div>
            ...
            <JobTable
              ...
              handleAdminForceRetry={(id) =>
                handleAdminAction(id, "force-retry")
              }
          ...
          >
            ...
            <div className="glass-panel p-10 rounded-[40px] border-white/5 relative overflow-hidden">
              <div className="flex items-center gap-6 mb-10 relative z-10">
                ...
                <div>
                  <h3 className="text-2xl font-black text-white tracking-tight mb-2 uppercase tracking-widest">
                    Architectural Analysis
                  </h3>
                  <p className="text-sm text-gray-500 max-w-xl">
                    Deep cognitive scan of the structural integrity and logic
                    flow across the Proedit ecosystem.
                  </p>
                </div>
              </div>
              <div className="space-y-6 relative z-10">
                {architectAnswer && (
                  <div className="p-8 bg-white/[0.03] rounded-[32px] border border-white/10 animate-in fade-in zoom-in-95">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <span className="text-[10px] font-black text-brand-cyan uppercase tracking-[0.4em]">
                    ...
                    {architectWorkflowJson && (
                      <div className="mt-8 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            const blob = new Blob([architectWorkflowJson], {
                              type: "application/json",
                            });
                            const url = URL.createObjectURL(blob);
                            const a = document.createElement("a");
                            a.href = url;
                            a.download = `proedit_n8n_workflow_${Date.now()}.json`;
                ...
                <form
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      setArchitectWorkflowJson(
                        res.is_workflow ? res.workflow_json : null,
                      );
                    } catch {
                      toast.error("Architect node timed out");
                    } finally {
                ...
                >
                  ...
                  <Button
                    ...
                    className="px-10 font-black text-xs uppercase tracking-[0.3em]"
                  >
                    CONSULT
                  </Button>
                </form>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <select
                    ...
                    onChange={(e) =>
                      setCreditForm((s) => ({ ...s, userId: e.target.value }))
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCreditForm((s) => ({ ...s, reason: e.target.value }))
                    }
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {ledger.map((entry) => (
                      <tr
                        key={entry.id}
                        className="hover:bg-white/[0.02] transition-colors border-l-2 border-transparent hover:border-brand-cyan"
                      >
                        <td className="px-8 py-6">
                          <div className="text-xs font-black text-white uppercase tracking-widest">
                            {entry.user_email}
                          </div>
                          <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1 opacity-50">
                            {entry.source}
                          </div>
                        </td>
                        <td
                          className={`px-8 py-6 text-sm font-black italic tracking-tighter ${entry.delta >= 0 ? "text-emerald-400" : "text-red-400"}`}
                        >
                          {entry.delta >= 0 ? "+" : ""}
                          {entry.delta}
                        </td>
                        <td className="px-8 py-6 text-sm font-black text-amber-400 font-mono">
                          {entry.balance_after} CR
                        </td>
                        <td className="px-8 py-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest max-w-xs truncate">
                          {entry.reason || "SYSTEM_EVENT"}
                        </td>
                        <td className="px-8 py-6 text-[10px] font-bold text-gray-600 uppercase tracking-widest whitespace-nowrap">
