  ...
  useEffect(() => {
    ...
    const checkHealth = async () => {
      try {
        ...
        // Check Worker
        const workerRes = await apiRequest<{ status: string }>(
          "/admin/worker-status",
          { auth: true },
        );
        if (!cancelled) setWorkerOnline(workerRes.status === "online");
      } catch {
        if (!cancelled) {
          setApiHealthy(false);
      ...
      <div className="hidden md:flex items-center gap-2 text-sm">
        {segments.map((segment, i) => (
          <div key={segment} className="flex items-center gap-2">
            ...
            <span
              className={`capitalize font-display tracking-tight ${i === segments.length - 1 ? "text-white font-bold" : "text-gray-500"}`}
            >
              {segment}
            </span>
          </div>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <div className="hidden sm:flex items-center gap-3 px-3 py-1.5 rounded-full glass-sm text-[10px] h-9">
          <div className="flex items-center gap-1.5 pr-3">
            <div
              className={`w-1.5 h-1.5 rounded-full ${apiHealthy ? "bg-brand-cyan shadow-[0_0_8px_rgba(0,229,255,0.5)]" : "bg-red-400"}`}
            />
            <span className="text-gray-400 uppercase tracking-widest font-bold">
              API
            </span>
          </div>
          <div className="flex items-center gap-1.5 border-l border-white/5 pl-3 pr-3">
            <div
              className={`w-1.5 h-1.5 rounded-full ${workerOnline ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]" : "bg-orange-400 shadow-[0_0_8px_rgba(251,146,60,0.5)]"}`}
            />
            <span className="text-gray-400 uppercase tracking-widest font-bold">
              Worker
            </span>
          </div>
          <span className="font-mono text-gray-300 border-l border-white/5 pl-3">
            {clock}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 px-3 py-1.5 rounded-full glass-md border-white/5 h-9">
          ...
          <button
            onClick={() => {
              ...
              SafeStorage.setItem("local_only_mode", String(newState));
            }}
            className={`w-8 h-4 rounded-full relative transition-colors duration-300 ${localOnly ? "bg-brand-cyan" : "bg-white/10"}`}
          >
            <div
              className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all duration-300 shadow-sm ${localOnly ? "left-4" : "left-0.5"}`}
            />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full glass-md border-brand-cyan/20 text-[10px] font-bold text-slate-300 h-9">
          ...
          <button
            className="ml-1 text-white/50 hover:text-brand-cyan transition-colors"
            title="Add Credits"
          >
            +
          </button>
